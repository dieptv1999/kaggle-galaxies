...
def threaded_gen(source_gen, buffer_size=2, sleep_time=1):
    ...
    thread = threading.Thread(target=_generation_thread,
                              args=(source_gen, buffer, buffer_lock, buffer_size, sleep_time))
    ...
    while True:
        ...
        if buffer_is_empty:
            ...
            if not thread.isAlive():
                print
                "buffer is empty and loading thread is finished, exiting"
                break
...
def im_normhist(img,
                num_bins=256):  # from http://www.janeriksolem.net/2009/06/histogram-equalization-with-python-and.html
...
def chunk_gcn(chunk, rescale=1.0):
    means = chunk.reshape(chunk.shape[0], chunk.shape[1] * chunk.shape[2], chunk.shape[3]).mean(1).reshape(
        chunk.shape[0], 1, 1, chunk.shape[3])
    chunk -= means

    stds = chunk.reshape(chunk.shape[0], chunk.shape[1] * chunk.shape[2], chunk.shape[3]).std(1).reshape(chunk.shape[0],
                                                                                                         1, 1,
                                                                                                         chunk.shape[3])
    chunk /= stds
...
def buffered_gen(source_gen, buffer_size=2, sleep_time=1):
    ...
    def _buffered_generation_thread(source_gen, buffer):
        while True:
            ...
            while buffer.full():
                print
                "DEBUG: buffer is full, waiting to generate more data."
                time.sleep(sleep_time)
