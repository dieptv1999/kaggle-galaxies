...
def create_train_gen():
    ...
    data_gen_train = ra.realtime_fixed_augmented_data_gen(train_indices, 'train',
                                                          ds_transforms=ds_transforms, chunk_size=CHUNK_SIZE,
                                                          target_sizes=input_sizes)
...
xs_valid = [[] for _ in range(num_input_representations)]
...
l1a = cc_layers.CudaConvnetConv2DLayer(l0s, n_filters=32, filter_size=6, weights_std=0.01, init_bias_value=0.1,
                                       dropout=0.0, partial_sum=1, untie_biases=True)
l1 = cc_layers.CudaConvnetPooling2DLayer(l1a, pool_size=2)

l2a = cc_layers.CudaConvnetConv2DLayer(l1, n_filters=64, filter_size=5, weights_std=0.01, init_bias_value=0.1,
                                       dropout=0.0, partial_sum=1, untie_biases=True)
l2 = cc_layers.CudaConvnetPooling2DLayer(l2a, pool_size=2)

l3a = cc_layers.CudaConvnetConv2DLayer(l2, n_filters=128, filter_size=3, weights_std=0.01, init_bias_value=0.1,
                                       dropout=0.0, partial_sum=1, untie_biases=True)
l3b = cc_layers.CudaConvnetConv2DLayer(l3a, n_filters=128, filter_size=3, pad=0, weights_std=0.1, init_bias_value=0.1,
                                       dropout=0.0, partial_sum=1, untie_biases=True)
...
l4a = layers.DenseLayer(j3, n_outputs=4096, weights_std=0.001, init_bias_value=0.01, dropout=0.5,
                        nonlinearity=layers.identity)
l4b = layers.FeatureMaxPoolingLayer(l4a, pool_size=2, feature_dim=1, implementation='reshape')
l4c = layers.DenseLayer(l4b, n_outputs=4096, weights_std=0.001, init_bias_value=0.01, dropout=0.5,
                        nonlinearity=layers.identity)
...
l5 = layers.DenseLayer(l4, n_outputs=37, weights_std=0.01, init_bias_value=0.1, dropout=0.5,
                       nonlinearity=layers.identity)

# l6 = layers.OutputLayer(l5, error_measure='mse')
l6 = custom.OptimisedDivGalaxyOutputLayer(
    l5)  # this incorporates the constraints on the output (probabilities sum to one, weighting, etc.)
...
xs_shared = [theano.shared(np.zeros((1, 1, 1, 1), dtype=theano.config.floatX)) for _ in
             range(num_input_representations)]
...
updates_nonorm = layers.gen_updates_nesterov_momentum_no_bias_decay(train_loss_nonorm, all_parameters,
                                                                    all_bias_parameters, learning_rate=learning_rate,
                                                                    momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
updates = layers.gen_updates_nesterov_momentum_no_bias_decay(train_loss, all_parameters, all_bias_parameters,
...
compute_output = theano.function([idx], l6.predictions(dropout_active=False), givens=givens,
                                 on_unused_input='ignore')  # not using the labels, so theano complains
...
rescaling_updates = l4a.rescaling_updates(0.4) + l4c.rescaling_updates(
    0.23)  # constants were determined by looking at histograms.
...
for e in range(NUM_CHUNKS):
    ...
    if e in LEARNING_RATE_SCHEDULE:
        ...
        print
        "  setting learning rate to %.6f" % current_lr
    ...
    print
    "  mean training loss (RMSE):\t\t%.6f" % mean_train_loss
    ...
    if ((e + 1) % VALIDATE_EVERY) == 0:
        ...
        print
        "  mean validation loss (RMSE):\t\t%.6f" % mean_valid_loss
    ...
    print
    "  %s since start (%.2f s)" % (load_data.hms(time_since_start), time_since_prev)
    print
    "  estimated %s to go (ETA: %s)" % (load_data.hms(est_time_left), eta_str)
    print

del chunk_data, xs_chunk, x_chunk, y_chunk, xs_valid, x_valid  # memory cleanup

print
"Compute predictions on validation set for analysis in batches"
predictions_list = []
for b in range(num_batches_valid):
    # if b % 1000 == 0:
    #     print "  batch %d/%d" % (b + 1, num_batches_valid)
...
print
"Write validation set predictions to %s" % ANALYSIS_PATH
with open(ANALYSIS_PATH, 'w') as f:
    pickle.dump({
        'ids': valid_ids[:num_batches_valid * BATCH_SIZE],
        # note that we need to truncate the ids to a multiple of the batch size.
...
for e, (xs_chunk, chunk_length) in enumerate(create_test_gen()):
    ...
    num_batches_chunk = int(
        np.ceil(chunk_length / float(BATCH_SIZE)))  # need to round UP this time to account for all data
...
print
"Write predictions to %s" % TARGET_PATH
# test_ids = np.load("data/test_ids.npy")
